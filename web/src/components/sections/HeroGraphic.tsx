  ...
  return (
    <div className="relative w-full">
      <svg viewBox="0 0 460 260" className="w-full" role="img" aria-label="Schematic of data feeding AI agents that take governed actions">
        ...
        {nodes.map((n) => (
          <g key={n.label}>
            <rect x={n.x - 44} y={n.y - 16} width="88" height="32" rx="6" fill="#fff" stroke="#d3d3cd" strokeWidth="1.5" />
        ...
        {/* agent core */}
        <rect x="206" y="98" width="48" height="64" rx="10" fill="#0a0a0b" />
        <text x="230" y="126" textAnchor="middle" fill="#fff" fontSize="10" fontFamily="var(--font-mono)">
          AGENT
        </text>
        <text x="230" y="140" textAnchor="middle" fill="#a4a4ac" fontSize="9" fontFamily="var(--font-mono)">
          CORE
        </text>
        {!reduce ? (
          <motion.rect
        ...
        {actions.map((a, i) => (
          <g key={`act-${i}`}>
            <circle cx={a.x} cy={a.y} r="14" fill="#f6f6f3" stroke="#d3d3cd" strokeWidth="1.5" />
            <circle cx={a.x} cy={a.y} r="4" fill="#0a0a0b" />
          </g>
        ))}
